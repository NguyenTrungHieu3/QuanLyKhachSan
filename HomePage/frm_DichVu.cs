    ...
    public partial class frm_DichVu : Form
    {
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                DialogResult dg  = MessageBox.Show("Bạn có chắc chắn muốn xóa dịch vụ này không?", "Xác nhận", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                if (dg == DialogResult.Yes)
                {
                    string sql = $"DELETE FROM Services WHERE ServiceID = {txtServiceID.Text}";

                    int kqXoa = dc.ThemSuaXoa(sql);
        ...
        {
            txtServiceID.Text = dgvServices.CurrentRow.Cells["ServiceID"].Value.ToString();
            txtServiceName.Text = dgvServices.CurrentRow.Cells["ServiceName"].Value.ToString();
            txtDescription.Text = dgvServices.CurrentRow.Cells["Description"].Value != null
                ? dgvServices.CurrentRow.Cells["Description"].Value.ToString()
                : "";
